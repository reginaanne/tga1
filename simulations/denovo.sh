...
REC=$2 # rescaled recombination rate: 1e-6 or 1e-8
...
SLIM=../../sim_scripts/${DEMOG}_${FREQ_LABEL}_domestication_forward.slim
...
conda activate slim
echo $TREES
slim -d sel=$SEL -d rec=$REC -d "trees='$TREES'" -seed ${SIMID} $SLIM
conda deactivate
